...
uint8_t Equalizer = 0;               // Еквалайзер
...
uint32_t mp3_event_timer;            // Періодичність перевірки стану DFPlayer
uint32_t mp3_timer = 0;
uint8_t eff_volume = 8;                      // громкость воспроизведения
uint8_t mp3_receive_buf[10];
uint8_t mp3_delay = 750;                 // Затрімка між командами плеєру
...
// byte cycle_volume = 0; // ????
byte volume_cycle = 0;
byte volume_target = 0;
byte volume_level = 0;
byte songs = 0;

byte home_environment = 0;
byte projector_light = 0;

uint16_t weather_id = 800;
byte tod = 0;
byte clouds = 0;
...
#else
#ifdef USE_IOT
  //FastLED.addLeds<WS2812B, LED_PIN, COLOR_ORDER>(leds, NUM_LEDS + 9);                                   /*.setCorrection(TypicalLEDStrip)*/
...
void initConfigure() {
  ...
  sound = (jsonReadtoInt(configSetup, "sound") == 1);
  eff_volume = jsonReadtoInt(configSetup, "volume");
...
#endif
  if (eff_valid < 0xFF) GetGeolocation();
  LOG.print ("\neff_valid • ");
  LOG.println (eff_valid);
}
