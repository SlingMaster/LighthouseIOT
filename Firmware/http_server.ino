...
void testMatrix(uint8_t val) {
  if (val == 1) {
    ...
    if (WIDTH % 2 == 0U) {
      ...
      drawPixelXY(CENTER_X_MINOR, CENTER_Y_MAJOR, 0xFF3F00);
    }
    ...
    drawRec(0, 0, WIDTH, HEIGHT, 0xFF0000);
    FastLED.show();
    delay(2000);

    drawRec(0, 0, WIDTH, HEIGHT, 0x00FF00);
    FastLED.show();
    delay(2000);

    drawRec(0, 0, WIDTH, HEIGHT, 0x0000FF);
    FastLED.show();
    delay(2000);

    if (WIDTH > HEIGHT) {
      gradientHorizontal(0, 0, WIDTH, HEIGHT, 0, 255, 255, 255, 255U);
    } else {
...
String runCommand(byte cmd, uint8_t val, String valStr) {
  ...
  switch (cmd ) {
    ...
    case CMD_RANDOM:
      ...
      //      updateSets();
      weather_id += 101;
      if (weather_id > 900)  weather_id = 201;
      tod++;
      if (tod > 3) tod = 0;
      UpdateState();
      break;
    ...
    case CMD_CONFIG:
      ...
      body += "\"cfg\":" + readFile("config.json", 2048) + ",";
    ...
    /* mp3 control */
    case CMD_MP3_VOLUME: volume_cycle = val; SetPlayerVolume(volume_cycle); break;
    case CMD_MP3_PREV: PrevSong(); break;
    case CMD_MP3_STOP: StopSong(); runEffect(); break;
    case CMD_MP3_PLAY: PlaySong(); break;
    case CMD_MP3_NEXT: NextSong(); break;
