  ...
  uint16_t blinkHalfPeriod)                                 /* продолжительность одной вспышки в миллисекундах (полупериод) */
{
  ...
  FastLED.clear();
  /* установка яркости, которая была выставлена до вызова предупреждения */
  setBrightnessLevel();
  FastLED.delay(1);
...
#endif
  // принудительное отображение текущего эффекта (того, что был активен перед предупреждением)
  loadingFlag = true;
}
...
void autoSwapEff() {
  ...
  if (ONflag && eff_auto) {
    if (eff_rnd) {
      ...
      currentMode = random8(MODE_AMOUNT - 1);
      runEffect();
    } else {
...
void buttonTick() {
  ...
  if (clickCount == 1U) {
    if (dawnFlag) {
      ...
      ONflag = !ONflag;
      if (ONflag) PlaySoundAdvert(26, 50); /* dog */
      delay(1500);
      changePower();
    }
    updateSets();
